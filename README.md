...
1. **基礎知識** — 視覚暗号の基礎概念と特徴を解説
   - 1994年のNaor & Shamir論文に基づく基本原理の説明
   - 人間の目による直接復号の仕組み
   - 実用的な活用例の紹介

2. **暗号化** — 画像ファイルを2枚のランダムなシェアに分割
   - ファイル選択とドラッグ&ドロップ対応
   - しきい値スライダーによる白黒変換の調整
   - 2×2ピクセル拡張アルゴリズムによるシェア生成
   - 生成されたシェアの個別ダウンロード

3. **復号** — 2枚のシェアを重ねて秘密画像を復元
   - 複数シェアファイルの同時読み込み
   - 位置調整用オフセット機能（微調整対応）
   - Canvas API による即座の重ね合わせ表示

4. **理論** — アコーディオン形式で発展的な理論と応用例を詳解
   - 視覚暗号の基本原理（ピクセルパターンの図解付き）
   - 視覚暗号の特徴と安全性
   - VSSS（Visual Secret Sharing Scheme）への発展
   - 実用的応用例（認証・偽造防止・QRコード融合・教育）
   - 本プロジェクトの背景情報
